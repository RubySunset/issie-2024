...
(*
    XML comments added for functions that were originally from lines 157-259 and optimiseSymbol was fully refactored to more modular components
*)
...
introduced to encapsulate the logic for computing symbolic connection data,
 while the alignSym, tryResize, and scaleSymbol functions were created to isolate the symbol alignment and resizing operations.
...
let calculateSymConnData (wModel: BusWireT.Model) (symbol: Symbol): SymConnDataT =
    let folder (symConnData: SymConnDataT) _ wire =
        updateData wModel symbol symConnData () wire
    Map.fold folder { ConnMap = Map.empty } wModel.Wires
...
let scaleSymbol (wModel: BusWireT.Model) (symConnData: SymConnDataT) (symbol: Symbol): Symbol =
    let symCount =
        symConnData.ConnMap
        |> Map.toArray
...
let optimiseSymbol (wModel: BusWireT.Model) (symbol: Symbol) (boundingBoxes: Map<CommonTypes.ComponentId, BoundingBox>): BusWireT.Model =
    let symConnData = calculateSymConnData wModel symbol
    let scaledSymbol = scaleSymbol wModel symConnData symbol
