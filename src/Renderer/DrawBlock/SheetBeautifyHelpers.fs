...
let strictGetWiresInBox (model: SheetT.Model) (box: BoundingBox) : (Wire * int) list =
    ...
    let checkOverlapFolder (startPos: XYPos) (endPos: XYPos) (state: bool * int) (segment: Segment) : bool * int =
        ...
        (fst state || strictOverlap),
        if strictOverlap then
            segment.Index
        else
            snd state
...
let getSegmentVector (wire: Wire) (index: int) : XYPos =
    ...
    | IsEven, Vertical
    | IsOdd, Horizontal -> { X = 0.; Y = wire.Segments[index].Length }
    | IsEven, Horizontal
    | IsOdd, Vertical -> { X = wire.Segments[index].Length; Y = 0. }
...
let doesSegmentIntersectsBoundingBox (box: BoundingBox) (aSegment: ASegment) =
    ...
    let bbBottomRight = { X = box.TopLeft.X + box.W; Y = box.TopLeft.Y + box.H }
...
let indexList (list: List<'T>) =
    List.mapi (fun i list_element -> (i, list_element)) list
...
let mergeASegmentsOnXAxis (segments: list<ASegment>) =
    let rec mergeHelper acc current =
        function
...
let getVisibleASegments (segments: List<ASegment>) : (ASegment list * ASegment list) =
    let horizontalSegments, verticalSegments =
        List.partition (fun (segment: ASegment) -> (segment.Orientation = Horizontal)) segments
    let visibleHorizontalSegments =
        List.groupBy (fun segment -> segment.Start.Y) horizontalSegments
        |> List.map snd
        |> List.map (
            List.map (fun seg ->
                if seg.Start.X > seg.End.X then
                    { seg with
                        Start = { seg.Start with X = seg.End.X }
                        End = { seg.End with X = seg.Start.X } }
                else
                    seg)
        )
    ...
    let visibleVerticalSegments =
        List.groupBy (fun segment -> segment.Start.X) verticalSegments
        |> List.map snd
        |> List.map (
            List.map (fun seg ->
                if seg.Start.Y > seg.End.Y then
                    { seg with
                        Start = { seg.Start with Y = seg.End.Y }
                        End = { seg.End with Y = seg.Start.Y } }
                else
                    seg)
        )
        |> List.map (List.sortBy (fun seg -> seg.Start.X))
...
let doesRetrace (aSegments: List<ASegment>) =
    ...
    segmentsArray.Length = 3
    && segmentsArray[1].Segment.IsZero
    && (segmentsArray[0].Segment.Length
        * segmentsArray[2].Segment.Length) < 0
...
let getDimensionOfComponent (symbol: SymbolT.Symbol) =
...
let customComponentDimension_ =
    Lens.create getDimensionOfComponent setDimensionOfComponent
...
let setPositionOfSymbol (symbol: SymbolT.Symbol) (x: float) (y: float) = { symbol with Pos = { X = x; Y = y } }
...
let setPortsOfSymbol (symbol: SymbolT.Symbol) (edge: Edge) (ports: List<string>) =
    { symbol with
        PortMaps = { symbol.PortMaps with Order = Map.add edge ports symbol.PortMaps.Order } }
...
let getReverseStateOfMux2 (symbol: SymbolT.Symbol) =
    Option.defaultValue false symbol.ReversedInputPorts
...
let getPortPosition (port: Port) (symbol: SymbolT.Symbol) = symbol.Pos + getPortPos symbol port
...
let setRotationState (rotation: Rotation) (symbol: SymbolT.Symbol) =
    { symbol with
        SymbolT.Symbol.STransform = { symbol.STransform with Rotation = rotation } }
...
let setFlippedState (flip: bool) (symbol: SymbolT.Symbol) =
    { symbol with
        SymbolT.Symbol.STransform = { symbol.STransform with Flipped = flip } }
...
let numOfPairOfSymbolsIntersecting (sheet: SheetT.Model) =
    let boundingBoxes = Map.values sheet.BoundingBoxes |> Seq.toList
...
let numOfSegmentPairsCrossing (sheet: SheetT.Model) =
    ...
    let visibleHorizontalSegments, visibleVerticalSegments =
        getVisibleASegments allASegments
    ...
    let sameNetCount =
        List.filter
            (fun (horizontalSegment, verticalSegment) ->
                strictOverlap1D
...
let numOfVisibleRightAngles (sheet: SheetT.Model) =
    ...
    let allASegmentsPartitioned =
        List.map (List.partition (fun (segment: ASegment) -> (segment.Orientation = Horizontal))) allASegments
    List.map (fun (l1, l2) -> List.allPairs l1 l2) allASegmentsPartitioned
    |> List.map (
        List.filter (fun (horizontalSegment, verticalSegment) ->
            overlap1D
                (horizontalSegment.Start.X, horizontalSegment.End.X)
                (verticalSegment.Start.X, verticalSegment.End.X)
            && overlap1D
                (horizontalSegment.Start.Y, horizontalSegment.End.Y)
                (verticalSegment.Start.Y, verticalSegment.End.Y)
            && (sameNet sheet verticalSegment horizontalSegment))
    )
    |> (List.collect id)
    |> List.length
...
let findRetracingSegments (sheet: SheetT.Model) =
    ...
    let allASegmentsPartitioned = allASegments |> List.map (List.windowed 3)

    let allRetracingSegments =
        allASegmentsPartitioned
        |> List.map (fun outerList -> outerList |> List.filter doesRetrace)
        |> List.collect id

    let allStartingSegments = List.map (List.map (List.item 1)) allASegmentsPartitioned
    let allEndingSegments =
        List.map (List.map (fun segList -> List.item (List.length segList - 2) segList)) allASegmentsPartitioned

    let intersectingStartSegmentsWithSymbols =
        allStartingSegments
        |> List.filter doesRetrace
        |> List.map List.last
        |> List.filter (fun aSegment -> aSegmentIntersectsAnyBoundingBoxed aSegment symbolsBoundingBoxes)

    let intersectingEndSegmentsWithSymbols =
        allEndingSegments
        |> List.filter doesRetrace
        |> List.map List.head
        |> List.filter (fun aSegment -> aSegmentIntersectsAnyBoundingBoxed aSegment symbolsBoundingBoxes)
    ...
    (allRetracingSegments,
     intersectingStartSegmentsWithSymbols
     @ intersectingEndSegmentsWithSymbols)
